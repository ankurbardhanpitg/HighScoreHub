...
- `POST /api/scores` — body `{ "score": 12, "game": "flappy" }`, requires a signed-in user. The player name is taken from the account username. `game` can be `"flappy"`, `"2048"`, `"whack"`, `"pong"`, `"breakout"`, `"starwaves"`, `"bubble"`, `"snake"`, or `"tictactoe"` and defaults to `"flappy"`.
