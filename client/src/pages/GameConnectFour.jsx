...
import {
  ...
  getWinningLine,
  isDraw,
} from '../game/connectfour.js';

const BEST_KEY = 'highscorehub-connectfour-best';
const CPU_THINK_MS = 480;
const NEXT_ROUND_MS = 980;
...
export default function GameConnectFour() {
  ...
  const placeDisc = useCallback(
    (col, mark) => {
      if (playStateRef.current !== 'playing' || turnRef.current !== mark) {
        return false;
      }

      const placed = dropDisc(boardRef.current, col, mark);
      if (!placed) {
        return false;
      }
      ...
      setLastMove(placed.index);

      if (mark === PLAYER) {
        playTone(audioRef, { type: 'triangle', startFreq: 360, endFreq: 220, duration: 0.12, volume: 0.07 });
      } else {
        playTone(audioRef, { type: 'sine', startFreq: 280, endFreq: 180, duration: 0.12, volume: 0.06 });
      }

      const winner = getWinner(placed.board);
      if (winner) {
        finishRound(winner, getWinningLine(placed.board));
        return true;
      }

      if (isDraw(placed.board)) {
  ...
  useEffect(() => {
    ...
    cpuTimerRef.current = window.setTimeout(() => {
      const col = cpuMove(boardRef.current);
      if (col != null) {
        placeDisc(col, CPU);
      }
    }, CPU_THINK_MS);
  ...
  }, [playState, turn, roundResult, board, placeDisc]);
  ...
  const { handleQuit } = useGameExpand({
    onPause: () => {
      if (playStateRef.current === 'playing') {
        clearTimers();
      }
  ...
  async function handleSubmit(event) {
    ...
    try {
      const saved = await submitScore(playerScore, 'connectfour');
  ...
  const kicker =
    playState === 'waiting'
      ? `Press Start, then tap a column to drop a disc · first to ${WIN_SCORE} wins`
      : playState === 'playing'
        ? `You are pink · numbers 1–${COLS} also work · first to ${WIN_SCORE} beats the computer`
        : '';

  let turnText = 'Your turn — tap a column';
  ...
  return (
    <section className="game-wrap">
      ...
      <div className="puzzle-page c4-page">
        ...
        <div className="puzzle-stage c4-stage">
          <ConnectFourBoard
            board={board}
          ...
          {playState === 'waiting' && (
            <div className="overlay" onClick={startGame}>
              <div className="panel overlay-panel">
                <h2>Ready?</h2>
                <p>
                  Drop pink discs into a column. Get four in a row before the computer does. First to {WIN_SCORE}{' '}
                  wins the match!
                </p>
          ...
          {playState === 'ended' && (
            <div className="overlay">
              <div className={`panel overlay-panel${playerWonMatch ? ' overlay-win' : ''}`}>
                ...
                <p>
                  {playerWonMatch
                    ? 'Four in a row, five times — that’s a match win!'
                ...
                {user ? (
                  ...
                  <div className="score-form">
                    ...
                    <div className="actions">
                      <Link className="button" to="/signin" state={{ from: '/game/connectfour' }}>
                        Sign in
                      </Link>
                      <Link className="button button-pink" to="/signup" state={{ from: '/game/connectfour' }}>
                        Sign up
                      </Link>
                    </div>
                  </div>
                ...
                <div className="actions">
                  ...
                  {user ? (
                    <Link className="button button-secondary" to="/leaderboard?game=connectfour">
                      High scores
                    </Link>
                  ) : null}
                  <QuitGameButton onClick={handleQuit} />
